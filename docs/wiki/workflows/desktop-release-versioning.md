...
- 正式发布 tag 必须是 `vX.Y.Z`，并且 `X.Y.Z` 必须等于 `desktop/package.json` 的 `version`。正式 workflow 必须在打包前机器校验这一等式，不能只依赖 `v*` 触发规则。
- `beta` 分支只运行 Windows 包结构与安装器构建验证，不上传公开 GitHub Release；公开桌面包只能由 `main` 上的正式 tag 触发。
- 正式 Windows 包必须配置签名材料。Beta 验证可以构建未签名包，但正式 Release 不允许通过长期环境开关绕过签名门禁。
