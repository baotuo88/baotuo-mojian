...
`DirectorRunCommand` 的租约是执行权，不是完成事实。Worker 进程在 `leased` 或 `running` 状态退出后，恢复扫描只处理 `leaseExpiresAt < now` 的命令；可自动恢复的命令回到 `queued`，清除旧租约，并保留任务已有的 checkpoint、当前阶段和可用产物。恢复动作必须同时把关联 `NovelWorkflowTask` 置为可继续状态，不能通过重建任务来掩盖断点。

下一次 Worker 通过条件更新领取命令：只有仍为 `queued` 的记录可以被设置为新的 lease owner，因此多个 Worker 竞争同一命令时至多一个能成功 claim。`attempt` 在成功 claim 时递增，用于限制重复中断后的自动恢复次数；达到策略上限后转为人工恢复或失败，而不是无限重试。
...
- `DirectorRunCommand` 表达控制面命令、租约和幂等，不表达业务完成事实。命令执行器收到 `confirm_candidate` 后，必须先读取任务状态并写入 pipeline dispatch 记录，再委托 `NovelDirectorService.confirmCandidate`；导演服务负责业务确认，执行器不复制建书或规划逻辑。
...
- 浏览器通知提醒只消费“导演跟进”投影中的可处理分组：`needs_validation`、`exception`、`pending`。`auto_progress` 和 `replaced` 仍可显示在跟进中心，但不触发系统级通知。提醒开关属于当前浏览器本地偏好，并且必须受浏览器通知权限约束；前端不得为了弹窗重新推断 task status 或绕过跟进投影。
- 服务重启后不静默续跑长任务，应从真实产物断点判断可恢复范围，再由用户或策略确认继续。
- Worker 进程级恢复回归必须在命令真实进入 `running` 后终止进程，并等待 lease 过期后由新的 `DirectorWorker.tick()` 触发恢复与重新 claim。测试不得用正常 `stop()` 代替崩溃，因为正常停止会执行收尾逻辑；恢复后必须验证 `attempt` 增加、checkpoint 保留、执行器只运行一次，并继续通过统一章节 runtime 将可用正文和章节状态持久化后才算成功终结。
...
- 正式章节写作、审校和恢复上下文必须消费 `TimelineContextService.buildForChapter()` 生成的结构化时间线上下文。该上下文包含上一章事件、本章计划事件、开放钩子、禁止提前发生事件、时间锚点和状态变化；它与 canonical state、伏笔账本并列，不能因写作路径简化而置空。时间线不可用时可以降级继续，但必须记录诊断并让质量门检查保持可见。
...
- 新书自动导演创建的恢复入口是独立页面 `/novels/auto-director?taskId=<workflowTaskId>`。`taskId` 是前端 URL 的主参数；旧的 `/novels/create?mode=director&workflowTaskId=<id>` 只作为兼容输入，进入后应规范化到新页面。任务中心、恢复入口、候选确认链接和服务端 `sourceRoute` 都应指向新页面，保证刷新、浏览器重启或崩溃恢复后回到同一个候选/进度现场。
