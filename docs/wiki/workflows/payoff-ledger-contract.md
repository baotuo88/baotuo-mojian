...
- 章节引用上下文采用“卷级摘要 + 章节窗口”策略：卷级开放回报保留全卷摘要，章节 `payoffRefsJson` 默认只读取当前章节前 24 章至后 12 章。全书历史引用通过账本状态和 RAG 召回，禁止在每次同步中加载整本章节计划。
...
- **长篇章节同步逐渐变慢**：检查 `PayoffLedgerSyncService` 是否把章节窗口扩大为全量查询；卷级开放回报保持全量，章节引用应保持有限窗口。
