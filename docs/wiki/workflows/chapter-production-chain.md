...
- 自动导演章节执行的最小契约必须可独立验证：单章流调用 `ChapterRuntimeCoordinator`，批量范围先查询活动 pipeline，存在时恢复而不是重复创建，不存在时先创建保护快照再启动统一 pipeline。该契约测试可以使用确定性 Mock 内容，但不得在导演模块内直接调用 writer 或 repair service。
...
- 宽屏布局下左中右三栏应保持同高工作区，并在各自栏内独立滚动；移动端应折叠成分组区域，优先保留正文阅读和章节操作空间。
...
- 状态快照读取必须把章节顺序筛选下推到数据库：恢复某章之前的状态时直接查询 `sourceChapter.order < 当前章节序号` 的最新快照，不得先加载固定数量快照及其全部子状态再在内存筛选。快照历史保留策略需要独立的备份、恢复和兼容性设计后再执行清理。
- SQLite 快照维护遵循“先体检、再备份、后清理”：`pnpm --filter @ai-novel/server db:inspect` 只读输出数据库完整性、体积、状态快照和子状态规模；`db:prune-snapshots` 默认 dry-run，只有明确执行时才允许在已验证备份后清理自动 `NovelSnapshot`。`StoryStateSnapshot` 及其子状态暂不纳入自动删除范围。
...
- 章节编辑器（`client/src/pages/novels/components/chapterEditor/ChapterEditorShell.tsx`）的正文保存语义：修改停顿 2.5s 后自动保存到服务端；自动保存只做轻量更新（写该章内容 + 刷新编辑工作区查询），手动保存才触发整组查询失效与快照联动。
- 每次未保存修改都会同步写入 localStorage 草稿缓存（键 `dsh:chapter-draft:{novelId}:{chapterId}`）；切换章节时未保存草稿先落缓存，返回该章时若缓存内容与服务端不一致则恢复缓存并标记未保存状态；保存成功后清除缓存。
- 有未保存修改时拦截 `beforeunload`；自动保存失败保留本地草稿并在控制台告警，状态条显示错误。
- 新增编辑器保存路径时必须保持这三层（自动保存 / 本地缓存兜底 / 离开拦截），任何一层缺失都会回到"长章手稿静默丢失"的失败模式。
