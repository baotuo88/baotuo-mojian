...
项目的 README 已经承载了完整功能说明、开发记录、运行方式和截图，但它更适合已经进入仓库的读者。公开传播入口需要在更短时间内说明三件事：这个项目解决什么问题、长篇小说生产链怎样推进、访问者下一步是部署 Web 端还是查看源码。
...
- `docs/public/introduction.md`：项目是什么、适合谁、核心能力、长篇生产链和获取入口。
- `docs/public/installation.md`：Docker Compose 部署、模型连接和 Qdrant 可选配置。
