...
- `client/src/pages/novels/components/` 根目录只保留跨域入口组件（编辑视图容器、任务抽屉、运行指示器等），域内组件按业务域归入 `autoDirector/`、`chapterExecution/`、`characterAsset/`、`characterPanel/`、`workspace/`、`outline/`、`pipeline/`、`novelWorld/`、`storyMacroPlan/`、`takeover/`、`basicInfoForm/` 等责任目录；新增组件先选域再落位，不在根目录继续堆平铺文件。`client/tests/` 中的契约测试按源码路径断言，移动组件时必须同步更新测试内的路径引用。
