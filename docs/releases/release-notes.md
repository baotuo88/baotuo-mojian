...
### 2026-09-04（长篇生产恢复与任务可见性）

- 自动创作在后台执行器重启、重复继续或租约切换时，会从最近安全进度接续，避免同一章节被重复写入；任务只有真实执行完成后才会显示成功。
- 章节流水线在刷新或重新打开小说后仍会显示正在运行的任务、阶段和进度；启动失败会给出可执行的下一步提示，减少重复点击和无效等待。
- 长篇生产验收覆盖 100 章、4 卷、跨阶段摘要、状态快照、事实与伏笔回收，帮助确认整本书的连续推进证据可追踪。
- 手动创建项目遇到网络或服务错误时，会保留当前填写内容并显示可理解的恢复提示，减少重复填写和被留在无反馈页面的情况。
- 手动创建项目支持安全重试：网络中断后再次提交会复用原项目和创作任务，避免重复创建。

### 2026-09-04（跨卷剧情承接）

- 长篇写作跨卷更连贯：每一卷写完后，AI 会自动记下「这一卷真正发生了什么」——主角的推进、角色关系变化、还没解决的悬念、已经确立不可更改的事实；写下一卷时会自动承接这些内容，避免忘掉前卷剧情、重复旧情节或漏掉伏笔。

### 2026-09-02（超长篇写作保护）

- 章节编辑器现在会自动保存：写作停顿约两秒半后自动保存到服务端，同时在浏览器本地保留一份草稿副本；误触返回、切换章节或刷新页面时未保存内容不再丢失，页面关闭前会弹出确认提示。
- 整章生成对超长章节不再中途截断：生成请求的输出预算与章节目标字数联动，避免长章节写到一半被硬性截断。
- 全自动写作在模型连接异常挂起时会自动报错并进入既有的恢复流程，不再出现任务看似在运行、实际已卡住的静默停滞。
- 长篇续写的记忆更稳定：写到数百章时，最近的剧情线索与已完成事项不再因章节累积而被旧记录挤掉，续写连贯性更好。

### 2026-09-02（聚焦 Web 端）

- 项目现在只提供 Web 端形态：使用 Docker Compose 一键部署后，通过浏览器访问和创作，Windows 桌面安装包与桌面内更新入口移除。
- 设置中的"桌面与维护"调整为"系统维护"：作品数据保存在服务端，版本更新通过重新部署完成，无需在页面里处理安装包。
- 介绍站与安装文档的下载入口改为部署指南，新用户可以直接从 Web 端开始创作。
- 清理桌面专用依赖与构建脚本，仓库体积和安装体积同步减小。

### 2026-09-01（Docker Compose 部署）

- 复制根目录 `.env.example` 为 `.env` 并设置数据库密码后，即可执行 `docker compose up -d --build` 启动全栈服务。
- 数据库连接信息由模板中的数据库配置自动生成，只需修改一处密码即可完成配置，避免多处填写不一致导致服务无法启动。
- 新增一键启动的 Web、API 和 PostgreSQL 部署方式，默认通过 Web 同源访问，减少部署时的地址配置。
- 作品数据库、生成图片和可选知识库数据使用持久化卷，停止服务或更新镜像不会清空创作内容。
- 首次启动会先准备数据库结构，数据库未就绪时不会启动创作服务；知识库向量服务可按需启用。
- 提供环境变量模板、健康检查、更新、备份和公网部署边界说明，便于从本地体验迁移到受控服务器；部署目录需使用英文路径。

### 2026-08-31（自动创作恢复与发布稳定性）

- 自动创作后台进程意外退出后，会从已保存进度接续当前任务并继续完成章节；已有正文、章节状态和恢复位置保持不变。
- 多个后台执行器同时收到同一任务时，只会由一个执行器继续处理，避免重复生成、重复保存或并行推进同一章节。
- 恢复后的章节仍会经过统一写作与保存流程，正文完成后进入待审状态，不会只显示任务完成却没有实际内容。
- 开发数据库恢复增加更严格的完整性检查、执行前备份和恢复后校验，损坏来源会在写入前停止，降低本地作品数据被错误覆盖的风险。
- Beta 构建会执行更完整的服务端、客户端和桌面包验证；正式桌面发布只接受与应用版本一致的发布标识，并要求配置 Windows 签名材料。
- 超长篇章节生成会继续参考前章事件、本章计划、开放钩子、时间锚点和禁止提前发生的后续事件，减少时间线跳跃、伏笔过早揭示和章节承接断裂。
- 长篇角色会携带当前认知、计划、误判和依据参与章节创作；伏笔会保留从铺垫、触碰到回收的关键章节证据，降低人物突然全知、动机漂移和伏笔回收缺少铺垫的问题。
- 每卷会按照建立承诺、升级阻力、收束支线和完成高潮的阶段指导章节规划；连续多个节点强度没有变化时，AI 会主动安排可见进展，减少超长篇中段原地消耗。
- 未启用知识库检索时，系统诊断会明确显示该能力已关闭，不再把缺少 Embedding 密钥或向量数据库误报成 AI 服务故障。
...
### 2026-08-28（长篇小说稳定性与自动导演连续性）

- 产品名称更新为「宝拓墨间」，桌面端下载、公开介绍站与开源仓库入口统一指向新的项目地址。
- 旧项目现在可以正常进入章节审校和继续创作：角色状态、卷级方案与章节规划信息之间不会再因为来源标记或过期计划而卡住，从历史状态继续时会直接定位到该继续的位置。
- 自动导演与章节生产在中断、重规划和恢复时更稳定：局部章节计划失配会自动调整后续安排并继续，已保存正文保持不变，只有整书结构需要人工处理、正文无法保存或发生安全风险时才会暂停。
- 每本小说只保留最近 100 个在线状态快照，历史快照自动归档，长期创作时项目响应不会被陈旧状态拖慢。
- 世界观、角色与写法的参考上下文检索更稳定：任务按队列逐条处理、失败自动重试，任务中心可查看处理进度，减少生成时上下文缺失或重复处理。
- 自动导演创建页会展示当前采用的 AI 风险策略说明，任务中心的筛选栏重新排布，长期运营时找任务和处理异常更清晰。
