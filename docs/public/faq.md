...
Web 部署把小说数据库、任务状态、配置和生成产物保存在服务端。使用 Docker Compose 部署时，这些数据存放在 Docker 数据卷里：

- `baotuo-mojian-app_postgres_data`：小说、任务状态和配置所在的 PostgreSQL 数据库。
- `baotuo-mojian-app_image_storage`：生成图片等产物。

重要项目建议定期备份。使用 Docker 部署时可以直接备份数据库：
...
如果修改过默认数据库用户或库名，请同步替换命令参数。完整的数据卷说明和备份步骤见 Docker Compose 部署文档。

排查问题前**先备份再操作**，不要在没备份的情况下删除数据卷或重置数据。
...
重要小说建议定期导出或备份。不要在没有备份的情况下删除数据库、重置数据或清理服务端数据卷。需要排查问题时，先保留日志和数据库副本。
