...
- 章节编辑器支持自动保存：写作停顿约两秒半自动保存，本地同时保留草稿副本，误触返回或刷新不再丢失未保存内容，页面关闭前有确认提示。
- 超长章节生成不再中途截断：输出预算与章节目标字数联动；全自动写作在模型连接异常挂起时自动报错并进入恢复流程，不再静默卡住。
- 长篇续写记忆更稳定：最近剧情线索与已完成事项不再因章节累积被旧记录挤掉，续写连贯性更好。
- 项目聚焦 Web 端：Docker Compose 一键部署后通过浏览器创作，桌面安装包与桌面内更新入口移除。
- 设置中的"桌面与维护"调整为"系统维护"，作品数据保存在服务端，更新通过重新部署完成。
- 介绍站与安装文档入口改为部署指南，新用户直接从 Web 端开始。
- 清理桌面专用依赖与构建脚本，仓库体积和安装体积同步减小。

### 2026-09-01

- 复制根目录 `.env.example` 为 `.env` 并设置数据库密码后，即可执行 `docker compose up -d --build` 启动 Web、API 和 PostgreSQL，默认通过 Web 同源访问。
- 数据库连接信息由模板中的数据库配置自动生成，只需修改一处密码即可，避免多处配置不一致导致启动失败。
- 作品数据库、生成图片和可选知识库数据使用持久化卷，停止服务或更新镜像不会清空创作内容。
- 提供环境模板、健康检查、数据库初始化、更新、备份和公网部署边界说明；部署目录需使用英文路径。
