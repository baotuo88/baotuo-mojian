...
- 复制根目录 `.env.example` 为 `.env` 并完成配置后，即可执行 `./scripts/docker-compose-up.sh up -d --build` 启动 Web、API 和 PostgreSQL，默认通过 Web 同源访问。
...
- 多个后台执行器不会重复处理同一创作任务，避免重复生成、重复保存或并行推进同一章节。
- 恢复后的章节继续通过统一写作与保存流程完成正文，并进入待审状态。
- 本地作品数据恢复增加执行前备份、完整性检查和恢复后校验，损坏来源会在写入前停止。
- Beta 与正式桌面发布增加更完整的构建、版本和签名门禁。
- 超长篇章节生成会参考前章事件、本章计划、开放钩子、时间锚点和后续禁止事件，并携带参与角色的当前认知、误判及伏笔生命周期证据；卷级规划会按建立承诺、升级阻力、收束支线和完成高潮推进，帮助维持人物动机、时间线、长期回收与中段节奏。
- 关闭知识库检索后，AI 健康诊断会按实际开关状态判断，不会因未配置 Embedding 或 Qdrant 把可用的模型服务显示为故障。
- 章节生成会遵守用户设置的目标字数，不会因为场景数量较多而把单章预算自动放大。
- 超长篇伏笔会由 AI 判断是否属于同一条线索，再由系统保护既有账本和章节证据，减少同一伏笔被拆成多个条目的情况。
- 长篇运行记录会显示上下文分组、保留与压缩数量及 token 估算，便于判断章节创作的上下文成本。
