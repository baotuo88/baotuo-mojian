...
test("GET /api/rag/queue-overview returns queue health data", async () => {
  const originalGetQueueOverview = ragServices.ragIndexService.getQueueOverview;
  ragServices.ragIndexService.getQueueOverview = async () => ({
    counts: { queued: 2, running: 1, failed: 1 },
    total: 4,
    oldestQueuedAt: "2026-03-18T10:00:00.000Z",
    oldestQueuedRunAfter: "2026-03-18T10:00:00.000Z",
    latestFailure: {
      jobId: "rag-job-failed",
  ...
  const port = await listen(server);
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/rag/queue-overview`);
    assert.equal(response.status, 200);
    const payload = await response.json();
    assert.equal(payload.success, true);
    assert.equal(payload.data.counts.queued, 2);
    assert.equal(payload.data.latestFailure.jobId, "rag-job-failed");
    assert.equal(payload.data.workerConcurrency, 2);
  } finally {
    ragServices.ragIndexService.getQueueOverview = originalGetQueueOverview;
    await new Promise((resolve, reject) => server.close((error) => error ? reject(error) : resolve()));
  }
});
