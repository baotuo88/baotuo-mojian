...
test("listNovels attaches latest visible auto director summary, skips archived tasks, and exposes deduplicated token usage", async () => {
  const originals = {
    ...
    directorStepRunFindMany: prisma.directorStepRun.findMany,
  };
  ...
  prisma.directorStepRun.findMany = async () => ([
    { taskId: "task_visible", label: "正在生成角色阵容" },
  ]);
  ...
  try {
    ...
    prisma.directorStepRun.findMany = originals.directorStepRunFindMany;
  }
});
