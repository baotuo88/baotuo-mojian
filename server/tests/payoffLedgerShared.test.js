...
test("resolvePayoffLedgerSyncLedgerKey follows a validated AI reuse decision", () => {
  const existingRows = [createLedgerItem({
    ledgerKey: "canonical_hidden_rule",
    title: "隐藏规则",
    currentStatus: "pending_payoff",
  })];

  const resolvedKey = resolvePayoffLedgerSyncLedgerKey({
    ledgerKey: "new_wording",
    title: "系统背后的约束浮出水面",
    scopeType: "book",
    currentStatus: "hinted",
    riskSignals: [],
    identityDecision: {
      action: "reuse",
      existingLedgerKey: "canonical_hidden_rule",
      reason: "两项都指向同一套隐藏规则。",
    },
  }, existingRows);

  assert.equal(resolvedKey, "canonical_hidden_rule");
});

test("resolvePayoffLedgerSyncLedgerKey rejects invalid AI identity claims", () => {
  const terminalRows = [createLedgerItem({
    ledgerKey: "closed_rule",
    title: "已兑现规则",
    currentStatus: "paid_off",
  })];

  assert.throws(() => resolvePayoffLedgerSyncLedgerKey({
    ledgerKey: "new_rule",
    title: "新规则",
    scopeType: "book",
    currentStatus: "hinted",
    ...
    identityDecision: {
      action: "reuse",
      existingLedgerKey: "missing_rule",
      reason: "错误候选。",
    },
  }, terminalRows), /不存在/);
  assert.throws(() => resolvePayoffLedgerSyncLedgerKey({
