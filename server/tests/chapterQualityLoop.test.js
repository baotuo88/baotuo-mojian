...
test("buildChapterQualityLoopChapterUpdate clears stale repair state after a valid repair recheck", () => {
  ...
  }, assessment, "repair_recheck", null, null, score());
  ...
  assert.equal(update.qualityScore, 87);
  assert.equal(update.continuityScore, 88);
  assert.equal(update.characterScore, 85);
  assert.equal(update.pacingScore, 86);
  assert.match(update.repairHistory, /source=repair_recheck/);
});
