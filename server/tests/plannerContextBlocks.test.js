...
function createInput() {
  return {
    ...
    timelineContext: {
      ...
      previousEvents: [{ id: "event-4", title: "账本异常", summary: "主角确认账本被改动。", chapterIndex: 4, storyTimeLabel: "第二日午前" }],
      plannedEventsThisChapter: [{ id: "event-5", title: "第一次反压", summary: "本章必须形成可见收益。" }],
      openHooks: [], blockingHooks: [], softHooks: [], addressedHooks: [],
      forbiddenEvents: [{ id: "future-7", title: "幕后黑手身份揭示", reason: "第7章之后才能确认。" }],
      continuityRequirements: ["承接账本异常"],
      knownStateChanges: [],
    },
    payoffLedgerSummary: [
...
test("volume window exposes pacing phase and repeated flat-conflict warning", () => {
  const volumes = createInput().mappedVolumes.map((volume) => ({
    ...volume,
    openPayoffs: [],
    chapters: volume.chapters.map((chapter) => ({ ...chapter, conflictLevel: 3 })),
  }));
  volumes[0].chapters = [
    { chapterOrder: 1, title: "第1章", summary: "起势", conflictLevel: 3 },
    { chapterOrder: 2, title: "第2章", summary: "受压", conflictLevel: 3 },
    { chapterOrder: 3, title: "第3章", summary: "试探", conflictLevel: 3 },
    { chapterOrder: 4, title: "第4章", summary: "继续试探", conflictLevel: 3 },
    { chapterOrder: 5, title: "第5章", summary: "反压", conflictLevel: 4 },
  ];

  const middle = buildCurrentVolumeWindowSummary(volumes, 4);
  const ending = buildCurrentVolumeWindowSummary(volumes, 5);
  assert.match(middle, /卷级节奏阶段：收束支线、集中冲突并准备高潮/);
  assert.match(middle, /冲突强度持平/);
  assert.match(ending, /卷级节奏阶段：完成高潮、回收本卷承诺并留下下一卷接口/);
});
...
test("chapter planner context prioritizes framing, story macro and current volume window over legacy outline", () => {
  ...
  assert.match(byId.get("timeline_context").content, /账本异常/);
  assert.match(byId.get("timeline_context").content, /第一次反压/);
  assert.match(byId.get("timeline_context").content, /幕后黑手身份揭示/);
