...
test("artifact delta only applies active influence proposals in this chapter", async () => {
  ...
  try {
    const count = await service.applyCharacterDialogueInfluenceResolutions({
      ...
      resolutions: [
        {
          influenceId: "proposal-active",
          status: "applied",
          evidence: ["程秩确认退路后才潜入。"],
          confidence: 0.88,
        },
        {
          influenceId: "proposal-foreign",
          status: "applied",
          evidence: ["不应命中的提案。"],
          confidence: 0.9,
        },
...
test("artifact delta expires active influence proposals once their window has passed", async () => {
  const service = new ChapterArtifactDeltaService();
  const originalUpdateMany = prisma.characterDialogueInfluence.updateMany;
  const updateCalls = [];
  prisma.characterDialogueInfluence.updateMany = async (args) => {
