...
function buildSelectedContextGroupCounts(context: PromptRenderContext) {
  const groups = new Map<string, { group: string; count: number; estimatedTokens: number }>();
  for (const block of context.blocks) {
    const current = groups.get(block.group) ?? { group: block.group, count: 0, estimatedTokens: 0 };
    current.count += 1;
    current.estimatedTokens += block.estimatedTokens;
    groups.set(block.group, current);
  }
...
function buildPromptInvocationMeta(
  ...
  return {
    ...
    selectedContextBlockCount: context.selectedBlockIds.length,
