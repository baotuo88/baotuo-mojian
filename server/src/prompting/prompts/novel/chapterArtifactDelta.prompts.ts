...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "12a. payoffDeltas.identityDecision.action 只能使用 reuse 或 create；reuse 时 existingLedgerKey 必须原样引用已有伏笔账本中语义相同的 ledgerKey，create 时 existingLedgerKey 必须为空。",
    ...
    new HumanMessage([
      ...
      input.existingPayoffText || "暂无已有伏笔账本",
       "只能复用以下既有 ledgerKey；同一语义线索必须复用，不能改写成新 key：",
       input.existingPayoffItems.length > 0
         ? input.existingPayoffItems.map((item) => `- ledgerKey=${item.ledgerKey} | 标题=${item.title} | 状态=${item.currentStatus} | 摘要=${item.summary}`).join("\n")
         : "无",
      "",
  ...
  postValidate: (output, input) => {
    const existingByKey = new Map(input.existingPayoffItems.map((item) => [item.ledgerKey, item]));
    const claimedExistingKeys = new Set<string>();
    for (const payoff of output.payoffDeltas) {
      const claimedKey = payoff.identityDecision.existingLedgerKey?.trim();
      if (payoff.identityDecision.action === "reuse") {
        const existing = claimedKey ? existingByKey.get(claimedKey) : null;
        if (!existing || !claimedKey) {
          throw new Error(`伏笔 ${payoff.ledgerKey} 引用了不存在的既有账本项。`);
        }
        if (existing.currentStatus === "paid_off" || existing.currentStatus === "failed") {
          throw new Error(`伏笔 ${payoff.ledgerKey} 不能复用已终态账本项 ${claimedKey}。`);
        }
        if (claimedExistingKeys.has(claimedKey)) {
          throw new Error(`多个伏笔 delta 重复复用既有账本项：${claimedKey}`);
        }
        claimedExistingKeys.add(claimedKey);
      } else if (claimedKey) {
        throw new Error(`新建伏笔 ${payoff.ledgerKey} 不应引用既有账本项。`);
      }
    }
