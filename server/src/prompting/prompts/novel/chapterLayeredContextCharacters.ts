...
function buildMindGuidance(mind: GenerationContextPackage["characterMindStates"][number] | undefined): string | null {
  ...
  const parts = takeUnique([
    mind.currentInterpretation ? `角色理解（主观，不等同事实）：${compactText(mind.currentInterpretation)}` : "",
    mind.privateIntent ? `隐性意图：${compactText(mind.privateIntent)}` : "",
    mind.activePlan ? `当前计划：${compactText(mind.activePlan)}` : "",
    mind.emotionalStance ? `情绪立场：${compactText(mind.emotionalStance)}` : "",
    mind.actionTendency ? `受压反应：${compactText(mind.actionTendency)}` : "",
    mind.beliefs[0] ? `坚信：${compactText(mind.beliefs[0])}` : "",
    mind.misbeliefs[0] ? `可能误判：${compactText(mind.misbeliefs[0])}` : "",
    mind.evidence[0] ? `依据：${compactText(mind.evidence[0])}` : "",
  ], 6);
  return parts.length > 0 ? parts.join(" | ") : null;
}
