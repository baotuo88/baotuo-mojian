...
function buildPromptTracePreview(input: {
  ...
  return {
    ...
    selectedContextBlockCount: input.prepared.context.selectedBlockIds.length,
    droppedContextBlockCount: input.prepared.context.droppedBlockIds.length,
    summarizedContextBlockCount: input.prepared.context.summarizedBlockIds.length,
    selectedContextGroupCounts: input.prepared.context.blocks
      .filter((block) => input.prepared.context.selectedBlockIds.includes(block.id))
      .reduce<Array<{ group: string; count: number; estimatedTokens: number }>>((groups, block) => {
        const current = groups.find((item) => item.group === block.group);
        if (current) {
          current.count += 1;
          current.estimatedTokens += block.estimatedTokens;
        } else {
          groups.push({ group: block.group, count: 1, estimatedTokens: block.estimatedTokens });
        }
        return groups;
      }, []),
