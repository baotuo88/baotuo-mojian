  ...
  async syncChapterArtifacts(input: ChapterArtifactDeltaSyncInput): Promise<ChapterArtifactDeltaSyncResult> {
    ...
    const result = await runStructuredPrompt({
      ...
      promptInput: {
        ...
        existingPayoffItems: payoffRows.map((item) => ({
  ...
  private async applyPayoffDeltas(input: {
    ...
    await prisma.$transaction(async (tx) => {
      for (const item of input.output.payoffDeltas) {
        const identityRows = await tx.payoffLedgerItem.findMany({
          where: { novelId: input.novelId },
          select: {
            ledgerKey: true,
            title: true,
        ...
        const chapterLookup = createNovelChapterReferenceLookup(input.chapters);
        ...
        const setupChapterId = normalizedChapterRefs.setupChapterId;
        const payoffChapterId = normalizedChapterRefs.payoffChapterId;
