...
// 章节写作图与续写服务存在模块级循环引用，延迟实例化避免加载期拿到未初始化的类引用。
let continuationServiceInstance: NovelContinuationService | null = null;

function getContinuationService(): NovelContinuationService {
  if (!continuationServiceInstance) {
    continuationServiceInstance = new NovelContinuationService();
  }
  return continuationServiceInstance;
}
