...
interface PromptMetaRecord {
  ...
  selectedContextBlockCount?: number | null;
  droppedContextBlockCount?: number | null;
  summarizedContextBlockCount?: number | null;
  selectedContextGroupCounts?: Array<{ group: string; count: number; estimatedTokens: number }>;
...
export interface NovelPromptTraceContextGroupSummary {
  group: string;
  estimatedInputTokens: number;
  blockCount: number;
...
export interface NovelPromptTraceReport {
  ...
  contextGroups: NovelPromptTraceContextGroupSummary[];
}
...
export async function buildNovelPromptTraceReport(params: {
  ...
  const contextGroupAggregates = new Map<string, NovelPromptTraceContextGroupSummary>();
  ...
  for (const [requestId, promptMeta] of requestMetaById.entries()) {
    ...
    for (const group of promptMeta.selectedContextGroupCounts ?? []) {
      const row = contextGroupAggregates.get(group.group) ?? {
        group: group.group,
        estimatedInputTokens: 0,
        blockCount: 0,
        invocationCount: 0,
      };
      row.estimatedInputTokens += group.estimatedTokens;
      row.blockCount += group.count;
      row.invocationCount += 1;
      contextGroupAggregates.set(group.group, row);
    }
  ...
  return {
    ...
    contextGroups: [...contextGroupAggregates.values()]
      .sort((left, right) => right.estimatedInputTokens - left.estimatedInputTokens),
  };
}
