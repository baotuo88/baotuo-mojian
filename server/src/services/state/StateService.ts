...
import type { CharacterState, ForeshadowState, InformationState, RelationState } from "@prisma/client";
...
function deserializeArchivedSnapshot(archive: {
  id: string;
  novelId: string;
  ...
  const stored = JSON.parse(archive.snapshotJson) as {
    createdAt?: string;
    updatedAt?: string;
  ...
  const createdAt = stored.createdAt ? new Date(stored.createdAt) : archive.archivedAt;
  const updatedAt = stored.updatedAt ? new Date(stored.updatedAt) : createdAt;
  return {
    id: archive.id,
    novelId: archive.novelId,
    sourceChapterId: archive.sourceChapterId,
    summary: archive.summary,
    rawStateJson: stored.rawStateJson ?? null,
    createdAt,
    updatedAt,
    characterStates: stored.children?.CharacterState ?? [],
    relationStates: stored.children?.RelationState ?? [],
    informationStates: stored.children?.InformationState ?? [],
    foreshadowStates: stored.children?.ForeshadowState ?? [],
  };
}
...
export class StateService {
  ...
  async getChapterSnapshot(novelId: string, chapterId: string) {
    const snapshot = await prisma.storyStateSnapshot.findFirst({
    ...
    const archived = await prisma.storyStateSnapshotArchive.findFirst({
      where: { novelId, sourceChapterId: chapterId },
    });
    return archived ? deserializeArchivedSnapshot(archived) : null;
  }
  ...
  async getLatestSnapshotBeforeChapter(novelId: string, chapterOrder: number) {
    ...
    const archived = await prisma.storyStateSnapshotArchive.findFirst({
      where: {
        novelId,
        sourceChapterOrder: { lt: chapterOrder },
      },
      orderBy: [{ sourceChapterOrder: "desc" }, { archivedAt: "desc" }],
    });
    if (!archived) {
      return null;
    }

    return deserializeArchivedSnapshot(archived);
  }
