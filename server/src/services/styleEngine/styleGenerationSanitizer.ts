...
export function buildStyleEngineBlock(
  styleContext: StyleContextWithSanitizedProfile | null | undefined,
): string {
  const profile = styleContext?.sanitizedGenerationProfile;
  if (!profile) {
    return "";
  }
  const guidance = profile.writingGuidance.join("\n").trim();
  const forbidden = profile.forbiddenEntities.map(() => "[source-entity]").join("\n");
  return [
    "[style-engine]",
    guidance,
    forbidden ? `[source-entities]\n${forbidden}` : "",
  ].filter(Boolean).join("\n");
}
