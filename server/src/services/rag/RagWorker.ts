...
export class RagWorker {
  ...
  start(): void {
    ...
    this.logInfo("Worker started.", {
      ...
      concurrency: ragConfig.workerConcurrency,
  ...
  private async tick(): Promise<void> {
    try {
      const job = await this.ragIndexService.claimNextRunnableJob(`worker-${this.nextWorkerId++}`);
      if (!job) {
        return;
      }
  ...
  private async fillSlots(): Promise<void> {
    ...
    this.isFilling = true;
    try {
      while (this.activeTicks < ragConfig.workerConcurrency) {
        this.activeTicks += 1;
        void this.tick();
      }
    } finally {
