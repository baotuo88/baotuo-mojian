...
export class RagIndexService {
  ...
  async claimNextRunnableJob(workerId: string): Promise<RagIndexJob | null> {
    ...
    await this.updateJobProgress(job.id, {
      stage: "loading_source",
      label: "开始处理",
      detail: `RAG worker ${workerId} 已认领任务。`,
      percent: 0.02,
    });
    return job;
  }
  ...
  async getQueueOverview() {
    const [grouped, oldestQueued, latestFailure] = await Promise.all([
      prisma.ragIndexJob.groupBy({
        by: ["status"],
        _count: { _all: true },
      }),
      prisma.ragIndexJob.findFirst({
        where: { status: "queued" },
        orderBy: [{ runAfter: "asc" }, { createdAt: "asc" }],
        select: { createdAt: true, runAfter: true },
      }),
      prisma.ragIndexJob.findFirst({
        where: { status: "failed" },
        orderBy: { updatedAt: "desc" },
    ...
    return {
      ...
      latestFailure: latestFailure
        ...
        : null,
      workerConcurrency: ragConfig.workerConcurrency,
    };
  }
