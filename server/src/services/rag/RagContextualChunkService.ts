...
const defaultPromptRunner: ContextualPromptRunner = async (...args) => {
  const { runStructuredPrompt: promptRunner } = await import("../../prompting/core/promptRunner");
  return promptRunner(...args);
};
...
export class RagContextualChunkService {
  constructor(private readonly promptRunner: ContextualPromptRunner = defaultPromptRunner) {}
