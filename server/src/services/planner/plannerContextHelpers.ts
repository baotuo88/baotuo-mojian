...
export function buildCurrentVolumeWindowSummary(
  ...
  const orderedChapters = currentVolume.chapters
    .slice()
    .sort((left, right) => left.chapterOrder - right.chapterOrder);
  const chapterIndex = orderedChapters.findIndex((chapter) => chapter.chapterOrder === chapterOrder);
  const progress = currentVolume.chapters.length > 0
    ? (chapterIndex + 1) / currentVolume.chapters.length
    : 0;
  const pacingPhase = progress <= 0.2
    ? "建立卷级承诺与主要压力"
    : progress <= 0.55
      ? "升级阻力并兑现阶段收益"
      : progress <= 0.8
        ? "收束支线、集中冲突并准备高潮"
        : "完成高潮、回收本卷承诺并留下下一卷接口";
  const recentConflictLevels = orderedChapters
    .slice(Math.max(0, chapterIndex - 3), chapterIndex + 1)
    .map((chapter) => chapter.conflictLevel)
    .filter((level): level is number => typeof level === "number");
  const flatConflictRun = recentConflictLevels.length >= 3
    && recentConflictLevels.every((level) => level === recentConflictLevels[0]);

  return [
    `当前章节：第${chapterOrder}章（卷内位置 ${chapterIndex + 1}/${currentVolume.chapters.length}）`,
    `卷级节奏阶段：${pacingPhase}`,
    flatConflictRun ? `节奏提醒：最近 ${recentConflictLevels.length} 个卷内节点冲突强度持平，规划时应安排可见变化，避免继续原地消耗。` : "",
