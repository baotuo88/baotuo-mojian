...
export class PlannerService {
  ...
  async ensureChapterPlan(novelId: string, chapterId: string, options: PlannerOptions = {}) {
    ...
    if (existing && existing.scenes.length > 0) {
      ...
      // 旧计划（例如迁移或早期版本写入）缺少执行合约哈希，无法判断是否过期，
      // 应直接复用既有计划，避免每次上下文装配都触发一次 LLM 重新规划。
      if (!plannedContractHash) {
        return existing;
      }
  ...
  async generateChapterPlan(novelId: string, chapterId: string, options: GenerateChapterPlanOptions = {}) {
    ...
    const timelineContext = await timelineContextService.buildForChapter({
      ...
      console.warn("[planner] timeline context unavailable; continuing without timeline block.", error);
      return null;
    });
