...
type PayoffLedgerSyncCandidate = {
  ...
  statusReason?: string | null;
  identityDecision?: PayoffLedgerIdentityDecision | null;
};

export type ExistingLedgerIdentityRow = {
  ledgerKey: string;
  title: string;
  scopeType: "book" | "volume" | "chapter";
  currentStatus: PayoffLedgerStatus;
...
export function resolvePayoffLedgerSyncLedgerKey(
  ...
  const decision = item.identityDecision;
  if (decision?.action === "reuse") {
    const claimedKey = decision.existingLedgerKey?.trim();
    const existing = claimedKey ? existingRows.find((row) => row.ledgerKey === claimedKey) : null;
    if (!existing) {
  ...
  if (decision?.action === "create" && decision.existingLedgerKey) {
