...
function normalizeEvidence(
  ...
  const normalized = evidence
    ...
    .filter((item) => {
      const key = `${item.chapterId ?? ""}:${item.chapterOrder ?? ""}:${item.summary}`;
  ...
  const ordered = normalized
    .sort((left, right) => (left.chapterOrder ?? -1) - (right.chapterOrder ?? -1));
  if (ordered.length <= 4) {
    return ordered;
  }
  // 保留首次铺垫这一锚点，再保留最近三个生命周期节点，避免“最新证据”覆盖起因。
  return [ordered[0], ...ordered.slice(-3)];
}
