...
const PAYOFF_CHAPTER_WINDOW_BEFORE = 24;
...
export class PayoffLedgerSyncService {
  ...
  private async loadLedgerIdentityCandidates(novelId: string) {
  ...
  private async buildSyncPromptInput(novelId: string, options: PayoffLedgerSyncOptions) {
    ...
    const activeVolumeSummary = activeVolume
      ? [
          ...
          activeVolumeChapters.length > 0
            ? `当前窗口章节范围：${activeVolumeChapters[0]?.chapterOrder ?? "-"}-${activeVolumeChapters[activeVolumeChapters.length - 1]?.chapterOrder ?? "-"}`
            : "当前窗口章节范围：无",
        ].join("\n")
    ...
    return {
      ...
      promptInput: {
        ...
        existingLedgerItems: await this.loadLedgerIdentityCandidates(novelId),
