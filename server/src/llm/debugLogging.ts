...
function buildHeader(method: "invoke" | "stream" | "batch", meta: LLMDebugMeta): string {
  ...
  if (meta.promptMeta) {
    ...
    chunks.push(`selectedContextBlocks=${meta.promptMeta.selectedContextBlockCount}`);
    chunks.push(`droppedContextBlocks=${meta.promptMeta.droppedContextBlockCount}`);
    chunks.push(`summarizedContextBlocks=${meta.promptMeta.summarizedContextBlockCount}`);
    if (meta.promptMeta.selectedContextGroupCounts.length > 0) {
      chunks.push(`selectedContextGroups=${meta.promptMeta.selectedContextGroupCounts.map((item) => `${item.group}:${item.count}/${item.estimatedTokens}`).join(",")}`);
    }
