...
router.get("/queue-overview", async (_req, res, next) => {
  try {
    const data = await ragServices.ragIndexService.getQueueOverview();
    res.status(200).json({
      success: true,
      data,
      message: "RAG 队列概览已加载。",
...
router.get("/health", async (_req, res, next) => {
  try {
    if (!ragConfig.enabled) {
      const data = {
        enabled: false,
        embedding: { ok: true, disabled: true },
        qdrant: { ok: true, disabled: true },
        ok: true,
      };
      res.status(200).json({
        success: true,
        data,
        message: "RAG 已关闭，当前不需要 Embedding 或向量数据库。",
      } satisfies ApiResponse<typeof data>);
      return;
    }
