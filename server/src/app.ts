...
import { NovelPipelineRuntimeService } from "./services/novel";
...
import { assertProductionNetworkBoundary } from "./config/serverNetwork";
...
export async function startServer(options?: ServerStartOptions): Promise<StartedServer> {
  ...
  logServerReady(host, listeningPort);
  ...
  return {
    ...
    port: listeningPort,
    allowLan,
    url: createServerUrl(host, listeningPort),
    close: async () => {
