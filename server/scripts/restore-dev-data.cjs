...
function assertSqliteHealthy(db, label) {
  const result = db.prepare("PRAGMA quick_check").get();
  if (result?.quick_check !== "ok") {
    throw new Error(`${label} failed SQLite quick_check: ${result?.quick_check ?? "unknown result"}`);
  }
}
...
function backupTargetDatabase(targetPath) {
  ...
  const backupDb = new DatabaseSync(backupPath, { open: true, readOnly: true });
  try {
    assertSqliteHealthy(backupDb, "Target backup");
  } finally {
    backupDb.close();
  }

  return { backupPath, size: stats.size, sha256: sha256File(backupPath) };
}
...
function main() {
  ...
  try {
    assertSqliteHealthy(sourceDb, "Source database");
    assertSqliteHealthy(targetDb, "Target database");
